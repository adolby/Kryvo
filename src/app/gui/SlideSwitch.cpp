...
class Kryvo::SlideSwitch::SlideSwitchPrivate {
 public:
  SlideSwitchPrivate();
...
Kryvo::SlideSwitch::SlideSwitch(QWidget* parent)
...
Kryvo::SlideSwitch::~SlideSwitch() {
}

void Kryvo::SlideSwitch::setBackgroundPixmap(const QString& backgroundPath) {
  m->background = QPixmap(backgroundPath);
}

void Kryvo::SlideSwitch::setKnobPixmaps(const QString& knobEnabledPath,
                                 const QString& knobDisabledPath) {
  m->knobEnabled = QPixmap(knobEnabledPath);
  m->knobDisabled = QPixmap(knobDisabledPath);
}

void Kryvo::SlideSwitch::paintEvent(QPaintEvent* /*event*/) {
...
QSize Kryvo::SlideSwitch::sizeHint() const {
...
void Kryvo::SlideSwitch::mouseMoveEvent(QMouseEvent* event) {
  if(m->dragInProgress) {
...
void Kryvo::SlideSwitch::mousePressEvent(QMouseEvent* event) {
...
void Kryvo::SlideSwitch::mouseReleaseEvent(QMouseEvent* /*event*/) {
...
bool Kryvo::SlideSwitch::hitButton(const QPoint& pos) const {
  return buttonRect().contains(pos);
}

void Kryvo::SlideSwitch::setSwitchPosition(const int position) {
...
void Kryvo::SlideSwitch::updateSwitchPosition(const bool checked) {
  if (checked) {
    m->position = 100;
  }
  else {
...
QRectF Kryvo::SlideSwitch::buttonRect() const {
...
QRectF Kryvo::SlideSwitch::knobRect() const {
...
Kryvo::SlideSwitch::SlideSwitchPrivate::SlideSwitchPrivate()
